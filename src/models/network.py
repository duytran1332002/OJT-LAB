...
# reconstruct Fusion-in-Decoder class with T5
class FiDT5(nn.Module):
    """Fusion-in-Decoder Transformer model."""
    """An encoder-decoder Transformer model."""
    config: T5Config
    ...
    def encode(self,
        ...
        """Applies Transformer encoder-branch on the inputs."""
        cfg = self.config
        if encoder_input_tokens.ndim == 3:
            # reshape to (batch_size * n_passages, passage_length)
            batch_size, n_passages, seq_len = encoder_input_tokens.shape
            encoder_input_tokens = jnp.reshape(encoder_input_tokens, (batch_size * n_passages, seq_len))
        assert encoder_input_tokens.ndim == 2, (
            f'Expected `encoder_input_tokens` to be of shape (batch * n_passages, len). '
            f'Got {encoder_input_tokens.shape}')
    ...
    def decode(self,
        ...
        if encoder_input_tokens.ndim == 3:
            # reshape to (batch_size, n_passages * passage_length)
            batch_size, n_passages, seq_len = encoder_input_tokens.shape
            encoded = jnp.reshape(encoded, (batch_size, n_passages * seq_len, encoded.shape[-1]))
            encoder_input_tokens = jnp.reshape(encoder_input_tokens, (batch_size, n_passages * seq_len))
            if encoder_segment_ids is not None:
              encoder_segment_ids = jnp.reshape(encoder_segment_ids, (batch_size, n_passages * seq_len))
    ...
    def __call__(self,
        ...
        #Reshape encoded from (batch_size * n_passages, seq_len, emb_dim) to (batch_size, n_passages * seq_len, emb_dim)
        encoded = jnp.reshape(encoded, (batch_size, n_passages * seq_len, encoded.shape[-1]))
        encoder_input_tokens = jnp.reshape(encoder_input_tokens, (batch_size, n_passages * seq_len))
        if encoder_segment_ids is not None:
            encoder_segment_ids = jnp.reshape(encoder_segment_ids, (batch_size, n_passages * seq_len))
...
#     def __call__(self, encoder_input_tokens=None, encoder_mask=None, **kwargs, ):
#         # total_length = n_passages * passage_length
#         batch_size, total_length = encoder_input_tokens.shape
#         passage_length = total_length // self.n_passages
#         # reshape to (batch_size * n_passages, passage_length)
#         encoder_input_tokens = jnp.reshape(encoder_input_tokens, (batch_size * self.n_passages, passage_length))
#         encoder_mask = jnp.reshape(encoder_mask, (batch_size * self.n_passages, passage_length, passage_length))
#         # encode
#         outputs = self.encoder(encoder_input_tokens, encoder_mask, **kwargs)
#         outputs = (jnp.reshape(outputs[0], (batch_size, self.n_passages * passage_length, -1)), ) + outputs[1:]
#         return outputs
